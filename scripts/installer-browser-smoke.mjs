...
try {
  ...
  // The only expected error is the deliberate 401 from reusing the consumed link.
  const unexpected = consoleErrors.filter((e) => !e.includes('status of 401'));
  check('no unexpected browser console errors', unexpected.length === 0 && consoleErrors.length === 1);
  if (unexpected.length) results.push(...unexpected.map((e) => `  console: ${e}`));
} catch (error) {
